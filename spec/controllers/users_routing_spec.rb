...
describe UsersController do
  describe "route generation" do
    ...
    it "should route users's 'forgot_password' action correctly" do
      route_for(:controller => 'users', :action => 'forgot_password').should == "/forgot_password"
    end

    it "should route users's 'reset_password' action correctly" do
      route_for(:controller => 'users', :action => 'reset_password', :activation_code => 'XXX').should == "/reset_password/XXX"
    end
  ...
  describe "route recognition" do
    ...
    it "should generate params for users's edit action from GET /users/1/edit" do
      params_from(:get , '/users/1/edit').should == {:controller => 'users', :action => 'edit', :id => '1'}
		...
		it "should generate params for users's reset action from POST /forgot_password" do
      params_from(:post , '/forgot_password').should == {:controller => 'users', :action => 'forgot_password'}
    end

		it "should generate params for users's reset action from PUT /reset_password" do
      params_from(:put , '/reset_password/XXX').should == {:controller => 'users', :action => 'reset_password', :activation_code => 'XXX'}
    end
