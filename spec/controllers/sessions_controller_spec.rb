...
describe SessionsController do
        ...
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
              ...
              controller.stub!(:update_logins).and_return(true) # bypassing the filter
              if want_remember_me
            ...
            if (has_request_token == :valid)
              it 'doesn\'t remember cookie' do @user.should_not_receive(:handle_remember_cookie!); end
  ...
  it "openid authentication" do
    controller.stub!(:update_logins).and_return(true) # bypassing the filter
    @user.stub!(:remember_token?).and_return('valid_token') 
    @user.stub!(:remember_token)
    @user.stub!(:remember_token_expires_at).and_return(15.minutes.from_now)
    @user.stub!(:remember_me) 
    @user.stub!(:refresh_token) 
    @user.stub!(:forget_me)
    openid_url = 'http://openid.aol.com/bestdebates'
    @login_params[:openid_url] = openid_url
    @login_params[:open_id_complete] = openid_url
    controller.should_receive(:authenticate_with_open_id).with(openid_url, :required => [:nickname, :email]).once
    do_create
  end
