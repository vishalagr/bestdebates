...
describe Folder do
  ...
  it "should raise ActiveRecord::RecordNotFound when trying to access other user's message" do
    ...
    @msg.save!
    lambda { 
      @recipient_2.folders.find_message(@recipient_1.inbox.private_messages.first.id)
    }.should raise_error(ActiveRecord::RecordNotFound)
    msg = @recipient_1.folders.find_message(@recipient_1.inbox.private_messages.first.id )
