class PrivateMessagesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.js   {render :partial => 'messaging_block' , :locals => {:messages => @messages , :order => @order , :folder => params[:folder] || "inbox"}}
    end
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.rss  { render :template => "private_messages/show.rss.builder", :layout => false }
    end    
  ...
  def create   
    @message = PrivateMessage.new(params[:message])
    @message.author = current_user
    
    respond_to do |format|
      if @message.save
        flash[:notice] = 'message was successfully created.'
        format.html { redirect_to( private_messages_path+'?folder=outbox' ) }
  ...
  def reply     
    ...
    if( !params[:parent_msg].blank? )
      begin
        parent_msg = current_user.folders.find_message(params[:parent_msg]).private_message
      rescue ActiveRecord::RecordNotFound => e
  ...
  def markread
    ...
    if message.save
       respond_to do |format|
        format.html{ render :partial => "message", :locals => { :message => message }, :layout => false}
