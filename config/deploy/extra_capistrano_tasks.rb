...
Capistrano::Configuration.instance.load do
  ...
  after 'deploy', 'deploy:delayed_job_stop'
  after 'deploy', 'deploy:delayed_job_start'  
  after 'deploy','deploy:delayed_job_restart'
  ...
  namespace :deploy do
   ...
   desc "Update the crontab file"
    ...
    task :generate_assets, :roles => :app do
     #run "cd #{current_path} && ruby vendor/gems/jammit-0.3.0/bin/jammit "
    end
